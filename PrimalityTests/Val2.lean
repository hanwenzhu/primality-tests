...
theorem odd_oddPart [hn : NeZero n] : Odd (oddPart n) := by
  ...
  intro h
  apply pow_succ_padicValNat_not_dvd (p := 2) hn.out
  rw [Nat.pow_succ, ← val₂]
  convert mul_dvd_mul_left _ h
...
theorem val₂_of_dvd [hn : NeZero n] (h : m ∣ n) : val₂ m ≤ val₂ n :=
...
theorem oddPart_of_dvd [NeZero m] (h : m ∣ n) : oddPart m ∣ oddPart n := by
  ...
  rw [← even_iff_two_dvd, ← odd_iff_not_even]
  exact odd_oddPart
